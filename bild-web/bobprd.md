...
- Method: `POST`
- Headers: `Authorization: Bearer <supabase_jwt>`, `Content-Type: application/json`
- Body: `{ "project_id": "<uuid>", "message": "<user message string>" }`
...
- **Verify JWT:** Set `verify_jwt: true` so Supabase validates the JWT before invoking the function; the function still verifies project membership.
...
| Client: Bob screen | `app/(tabs)/project.tsx` (sync effect, handleSend, citations) |
